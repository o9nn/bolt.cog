...
export abstract class BaseAgent implements Agent {
  ...
  public context: CognitiveContext;
  ...
  updateKnowledge(experience: Experience): void {
    ...
    this.experienceHistory = this.experienceHistory.filter(
      exp => new Date(exp.situation).getTime() > cutoff
    );
  }
  ...
  private generateSuggestions(parsed: any): string[] {
    ...
    for (const capability of this.capabilities) {
      if (parsed.intent.actions.some((action: string) => 
        capability.inputTypes.includes(action) || 
        capability.name.toLowerCase().includes(action.toLowerCase())
      )) {
  ...
  protected findRelevantKnowledge(context: string[]): Knowledge[] {
    return this.knowledgeBase.filter(knowledge =>
      knowledge.context.some(ctx => 
